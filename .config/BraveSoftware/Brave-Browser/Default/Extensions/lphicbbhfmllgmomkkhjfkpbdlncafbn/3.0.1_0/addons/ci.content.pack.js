...
window.AddonsFramework.prototype.Browser = function () {
	...
	function d(a) {
		var b = { action: 'ci_browser_navigate' },
			d;
		for (d in a) b[d] = a[d];
		return b;
	}
...
window.AddonsFramework.prototype.Extension = function () {
	...
	this.fireEvent = function (a, b, c) {
		c ? chrome.runtime.sendMessage({ action: 'event', name: a, data: b }, c) : chrome.runtime.sendMessage({ action: 'event', name: a, data: b });
	...
	this.detachEvent = function (a, b) {
		if (d[a]) for (var c = 0; c < d[a].length; c++) b && b === d[a][c] && (delete d[a][c], (d[a][c] = null), d[a].splice(c, 1));
	};
	this.log = function () {
		console.log.apply(console, arguments);
	};
	chrome.runtime.onMessage.addListener(function (a, b, c) {
		if ('event' == a.action && ((b = a.name), (a = { data: a.data.data }), d[b]))
