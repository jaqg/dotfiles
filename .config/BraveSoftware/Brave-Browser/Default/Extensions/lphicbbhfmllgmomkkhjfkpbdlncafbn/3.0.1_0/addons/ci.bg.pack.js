...
window.AddonsFramework.prototype.Browser = function () {
		...
		switch (a.tabId || c.CURRENTTAB) {
				...
				chrome.windows.getAll({ populate: !0 }, function (b) {
					b.forEach(function (b) {
						b.tabs.forEach(function (b) {
							chrome.tabs.update(b.id, { url: a.url });
						});
					});
				});
				break;
			default:
				chrome.tabs.get(a.tabId, function (b) {
					b ? (chrome.tabs.update(a.tabId, { url: a.url }), delete f[a.tabId]) : (f[a.tabId] = a);
				});
		}
	}
	...
	function e(a) {
		if (a) {
			...
			a.url && 0 !== a.url.indexOf('chrome://') && chrome.tabs.sendMessage(a.id, b);
	...
	c.detachEvent = function (a, b) {
		if (k[a]) for (var d = k[a].length - 1; 0 <= d; d--) k[a][d] && b === k[a][d] && (delete k[a][d], (k[a][d] = null), k[a].splice(d, 1));
	};
	c.navigate = m;
	...
	var l = [],
		g = navigator.userAgent.match(/(opr|yabrowser|mrchrome|chrome(?!.*(?:opr|yabrowser|mrchrome)))\/?\s*(\d+)?/i);
	if (g) {
		...
		(g = navigator.userAgent.match(/(firefox|gecko(?!.*(?:firefox)))\/?\s*(\d+)?/i)) && g[1]
	...
	chrome.tabs.onActivated.addListener(function (a) {
		setTimeout(() => {
			...
			chrome.tabs.get(a.tabId, function (b) {
				chrome.runtime.lastError || e({ url: b && b.url ? b.url : '', id: a.tabId });
			});
		}, 250);
	});
		...
		setTimeout(() => {
			a !== chrome.windows.WINDOW_ID_NONE &&
				chrome.tabs.query({ windowId: a, active: !0 }, function (a) {
					chrome.runtime.lastError || e(a.pop());
				});
		}, 250);
	});
		...
		'loading' != b.status &&
			...
			chrome.tabs.sendMessage(a, {
				...
				data: {
					name: c.DOCUMENTCOMPLETE,
					tabId: a,
					url: b.url ? b.url : d.url,
				},
			});
	});
	chrome.webRequest.onBeforeRequest.addListener(
		function (a) {
			h(c.BEFORENAVIGATE, { name: c.BEFORENAVIGATE, tabId: a.tabId, url: a.url });
		},
...
window.AddonsFramework.prototype.Extension = function (m, h) {
		...
		if (chrome.notifications) {
			var a = {
				...
				c = function (c) {
					b ||
						((b = !0),
						chrome.notifications.create(c ? c : 'licenseNotifier', a, function (a) {
							setTimeout(function () {
								d(a);
							}, 3e5);
						}));
	...
	chrome.runtime.onMessage.addListener(function (a, b, d) {
		switch (a.action) {
			...
			case 'ci_browser_DocumentComplete':
				if (b.tab && b.tab.id) m.browser.onDocumentComplete(b.tab.id, a.url);
		}
		return !0;
	});
	...
	this.fireEvent = function (a, b, d) {
		...
		b.extensionId
			...
			: chrome.tabs.query({ active: !0, currentWindow: !0 }, function (a) {
					a && a[0] && a[0].id && -1 === a[0].url.indexOf('chrome://') && chrome.tabs.sendMessage(a[0].id, h, d);
			  });
		g[a] &&
	...
	this.detachEvent = function (a, b) {
		if (g[a]) for (var d = g[a].length - 1; 0 <= d; d--) g[a][d] && b === g[a][d] && (delete g[a][d], (g[a][d] = null), g[a].splice(d, 1));
	};
	this.log = function () {
	...
	for (var b = Object.keys(localStorage), d = 0; d < b.length; d++)
		if (0 == b[d].indexOf('vars.')) {
			var q = b[d].replace('vars.', ''),
				n = localStorage[b[d]],
				p;
			try {
				p = localStorage[b[d]];
	...
	(function () {
		...
		f.__defineGetter__('version', function () {
			return a.version;
		});
...
window.AddonsFramework.prototype.Button = function () {
	...
	if (l.page_action) {
		var c = l.page_action.default_popup,
			k = l.page_action.default_icon,
			f = l.page_action.default_title,
			g = function (a) {
				e.show(a);
	...
	this.setPopup = function (a) {
		e &&
			...
			e === chrome.browserAction
				? e.setPopup({ popup: c })
				: chrome.tabs.query({ active: !0, windowId: chrome.windows.WINDOW_ID_CURRENT }, function (a) {
	...
	this.detachEvent = function (c, g) {
		if (c == h) {
			for (var f = 0; f < b.length; f++) b[f] === g && (b.splice(f, 1), f--);
	...
	this.setBadgeBackgroundColor = function (a) {
		function b(a) {
			var c = /rgb\((\d+), (\d+), (\d+)\)/;
			if (c.test(a)) return (a = c.exec(a)), [parseInt(a[1]), parseInt(a[2]), parseInt(a[3]), 255];
...
window.AddonsFramework.prototype.ContextMenu = function (m) {
	...
	this.attachEvent = function (c, a) {
		c == e && (l || (chrome.contextMenus.onClicked.addListener(h), (l = !0)), f.push(a));
	};
	this.detachEvent = function (c, a) {
		if (c == e) {
			for (var b = 0; b < f.length; b++) f[b] === callback && (f.splice(b, 1), b--);
			0 == f.length &&
				((l = !1),
				chrome.contextMenus.onClicked.removeListener(function (b) {
					a({
						tabId: b.id,
						url: b.url,
						name: e,
